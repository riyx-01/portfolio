...
export function Zone5_Contact() {
  return (
    <group position={[0, 0, -140]}>
      ...
      <NightCityBackground zOffset={-140} />
      ...
      <mesh rotation={[-Math.PI/2, 0, 0]} position={[0, -3.3, 0]}>
        <ringGeometry args={[2.5, 2.6, 32]} /> {/* Reduced segments */}
    ...
    <group position={[0, 0, -10]}>
      {rings.map((ring, i) => (
        <mesh key={i} ref={el => { ringRefs.current[i] = el }}>
          <torusGeometry args={[ring.r, 0.02, 6, 32]} /> {/* Optimized geometry */}
